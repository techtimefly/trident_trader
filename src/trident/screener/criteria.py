...
``Decimal``; a ``None`` bound on :class:`ScreenCriteria` means "no bound on that
side", so ``ScreenCriteria()`` matches every candidate.

The screener deliberately exposes only the filters that matter for a personal
...
- **recent % change band** — ``min_change_pct`` / ``max_change_pct`` over the
  lookback window. Cheap to add because the data layer already has the bars.

That is the whole filter set on purpose — no market cap, sector, float, P/E, or
